...
async function run(): Promise<void> {
  ...
  try {
    ...
    // IMPORTANT:
    // Upsert will overwrite existing fields, including turning non-null columns into null.
    // That is how we "lost" prices/specs after a partial enrich run.
    // To keep data stable, omit nullable fields when we don't have a value.
    const upsertPayload = listings.map((listing) => {
      ...
      if (listing.images.length > 0) row.images = listing.images
    ...
    console.log(`Upserted ${listings.length} cars`)
  } catch (error) {
